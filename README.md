...
3. Launch your app (this will create the app and deploy it):
   ```bash
   fly launch
   ```
   - When prompted, choose a region close to your users
   - The app name will be auto-generated or you can specify one

4. Set your environment variables:
   ```bash
   fly secrets set CLIENT_ID=your_client_id
   fly secrets set CLIENT_SECRET=your_client_secret
   fly secrets set SMS_TOKEN=your_sms_token
   fly secrets set SESSION_SECRET=$(openssl rand -base64 32)
   ```

5. Set your redirect URI (replace `your-app-name` with your actual Fly.io app name):
   ```bash
   fly secrets set REDIRECT_URI=https://your-app-name.fly.dev/oauth-callback
   ```

6. Update your HubSpot app settings:
   - OAuth redirect URI: `https://your-app-name.fly.dev/oauth-callback`
   - Webhook URL: `https://your-app-name.fly.dev/hubspot/workflow/send-sms`
...
**Note:** The app is configured to always stay running (no auto-stop) to ensure webhooks work reliably.
# traefik-testing-
